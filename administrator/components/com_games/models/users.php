...
class GamesModelUsers extends GamesModelBase 
{
	...
	protected function _buildQueryWhere(&$query)
    {
        ...
        if ($filter) 
        {
            ...
            $where = array();
            $where[] = 'LOWER(tbl.gamer_id) LIKE '.$key;
		...
		if ($filter_name) 
        {
            ...
            $where = array();
            $where[] = 'LOWER(e.gamer_id) LIKE '.$key;
		 ...
		 if (strlen($filter_id_from))
        {
            ...
            {
                $query->where('tbl.gamer_id >= '.(int) $filter_id_from);  
            }
                else
            {
                $query->where('tbl.gamer_id = '.(int) $filter_id_from);
        ...
        if (strlen($filter_id_to))
        {
            $query->where('tbl.gamer_id <= '.(int) $filter_id_to);
        }
	...
	{
			$item->link = 'index.php?option=com_games&view=users&task=edit&id='.$item->gamer_id;
